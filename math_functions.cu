...
#define TENSORRT_KERNEL_LOOP(i, n) \
...
cudaError_t PReLUForward(const int count, const int channels, const int dim, const float* bottom_data,
  ...
  PReLU<<<TENSORRT_GET_BLOCKS(count), TENSORRT_BLOCK>>>(count,channels,dim,bottom_data,top_data,static_cast<const float*>(mDeviceKernel),div_factor);
